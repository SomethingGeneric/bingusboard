...
**MANDATORY: Always run tests locally before pushing changes.** Before committing or pushing any code changes, run `make server-test-sqlite` for Go backend changes or `cd webapp && npm run test` for frontend changes. This helps catch issues early and prevents CI failures. For comprehensive testing across all databases, run `make server-test` before submitting pull requests.
